...
export default {
  ...
  methods: {
    ...
    show() {
      var _this = this
      _this.addFireRout('data/巷道属性.json')
    },
    // 隐藏
    hide() {
       if (window.cesiumvariate._firedataSource) {
        for (let i = 0; i < window.cesiumvariate._firedataSource.length; i++) {
          viewer.dataSources.remove(window.cesiumvariate._firedataSource[i])
        }
        window.cesiumvariate._firedataSource = null
      }
    },
     addFireRout(url) {
      var tt = this
      window.cesiumvariate._firedataSource = []
    ...
    createFlowVolume2(_t, W, H, speed, coordList) {
      ...
      // var imageFile = 'img/red128.png' //箭头图片
      var imageFile = 'img/avoidingDisaster/27.png' //箭头图片
    ...
    return material;\n\
      ...
      window.cesiumvariate._firedataSource.push(new Cesium.CustomDataSource(Cesium.createGuid()))
      viewer.dataSources.add(window.cesiumvariate._firedataSource[window.cesiumvariate._firedataSource.length - 1])
      var custom_flowMesh = window.cesiumvariate._firedataSource[window.cesiumvariate._firedataSource.length - 1]._primitives.add(
