...
export default {
  ...
  methods: {
    ...
    // 显示
    show() {
      window.fireflow = new CTMap.Flow(window.viewer, this.rescueType, this.rescueDirect)
       // 生成避灾路线
      const width = 6 // 巷道宽度
      const height = 4 // 巷道高度
      ...
      if (window.g_hangdao && window.g_hangdao.Map.length>0){
        window.fireflow.getNeededRoute(window.g_hangdao, width, height, speed, imageFile, arrowDis, 2)
      }
    },
    // 隐藏
    hide() {
       if(window.fireflow){
        window.fireflow.remove();
        window.fireflow = null
      }
    }
