...
export default {
  ...
  methods: {
    ...
    show() {
      var _this = this
      _this.addGasRout('data/巷道属性.json')
    },
    // 隐藏
    hide() {
       if (window.cesiumvariate._gasdataSource) {
        for (let i = 0; i < window.cesiumvariate._gasdataSource.length; i++) {
          viewer.dataSources.remove(window.cesiumvariate._gasdataSource[i])
        }
        window.cesiumvariate._gasdataSource = null
      }
    },
     addGasRout(url) {
      var tt = this
      window.cesiumvariate._gasdataSource = []
    ...
    createFlowVolume2(_t, W, H, speed, coordList) {
      ...
      var imageFile = 'img/avoidingDisaster/20.png' //箭头图片
    ...
    return material;\n\
      ...
      window.cesiumvariate._gasdataSource.push(new Cesium.CustomDataSource(Cesium.createGuid()))
      viewer.dataSources.add(window.cesiumvariate._gasdataSource[window.cesiumvariate._gasdataSource.length - 1])
      var custom_flowMesh = window.cesiumvariate._gasdataSource[window.cesiumvariate._gasdataSource.length - 1]._primitives.add(
