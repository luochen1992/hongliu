...
export default {
  ...
  methods: {
    ...
    show() {
      var _this = this
      _this.addMaterialsFlow('data/巷道属性.json')
    },
    // 隐藏
    hide() {
      if (window.cesiumvariate._materdataSource) {
        for (let i = 0; i < window.cesiumvariate._materdataSource.length; i++) {
          viewer.dataSources.remove(window.cesiumvariate._materdataSource[i])
        }
        window.cesiumvariate._materdataSource = null
      }
      if (this.handler) {
    ...
    addMaterialsFlow(url) {
      var tt = this
      window.cesiumvariate._materdataSource = []
      $.get(url, {}, function (res) {
        ...
        for (var j = 0; j < json.Map.length; j++) {
          ...
          if (obj.ModelType == 'MModel_Passage') {
            if (obj.MaterialsRoute) {
              var W = obj.W*2
              var H = obj.H
              var xyz = obj.XYZ
              var coordList = []
              var xyzArr = obj.XYZ.split(';')
              if (obj.MaterialsRouteDirect) {
                //与巷道同向
    ...
    createFlowVolume2(_t, W, H, speed, coordList) {
      ...
      var imageFile = 'img/orange128.png' //箭头图片
      //var speed = 0.01;  //速度
    ...
    return material;\n\
      ...
      window.cesiumvariate._materdataSource.push(new Cesium.CustomDataSource(Cesium.createGuid()))
      // window.cesiumvariate._materdataSource = new Cesium.CustomDataSource(Cesium.createGuid())
      viewer.dataSources.add(window.cesiumvariate._materdataSource[window.cesiumvariate._materdataSource.length - 1])
      var custom_flowMesh = window.cesiumvariate._materdataSource[window.cesiumvariate._materdataSource.length - 1]._primitives.add(
