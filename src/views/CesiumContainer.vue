...
import searchTool from './children/bottomRightBar/searchTool.vue'
...
import CoalAnimation from './children/functionmenu/coalAnimation.vue'
import Coalflow from './children/functionmenu/coalflow.vue'
import Materialsflow from './children/functionmenu/materialsflow.vue'
import personsite from '../components/JCJK/personsite.vue'
import jiankongsite from '../components/JCJK/jiankongsite.vue'
import zongcaiAnimation from '../components/ZC/zongcaiAnimation.vue'
// import '../js/material'
export default {
  ...
  methods: {
    ...
    addGrowLine() {
      var position = [112.666679, 35.699997, 1.50,
        112.715782, 35.699989, 1.47,
        112.705994, 35.679314, 1.49,
        112.729988, 35.676205, 1.49,
        112.729973, 35.674404, 1.50,
        112.734169, 35.674397, 1.52,
        112.734154, 35.669800, 1.50,
        112.725014, 35.669807, 1.51,
        112.725136, 35.673183, 1.52,
        112.714981, 35.675266, 1.50,
        112.712860, 35.672646, 1.50,
        112.703995, 35.674438, 1.51,
        112.703201, 35.671791, 1.50,
        112.692116, 35.671661, 1.50,
        112.692169, 35.660225, 1.53,
        112.656815, 35.666321, 1.50,
        112.643036, 35.666340, 1.52,
        112.658394, 35.704113, 1.54,
        112.666656, 35.704159, 1.53,
        112.666679, 35.699997, 1.50]

      viewer.entities.add({
        name: '',
        polyline: {
          positions: Cesium.Cartesian3.fromDegreesArrayHeights(position),
          distanceDisplayCondition: new Cesium.DistanceDisplayCondition(5800, 10000),
          width: 18,
    ...
    addGroudOverview(){
      $.get("data/采空区.json",{},function(res)
      {
        ...
        {	
          ...
          for(var k=0;k<polygonXYZ.length-3;k+=3)
          {
            ...
            pnts.push(fromLocal(x,y,z-938));
          //  pntsText.push(fromLocal(x,y,z-937));
          }
          ...
          {	
            ...
            if(obj.ModelType=="MModel_Passage" )
            {
              ...
              for(var k=0;k<xyzArr.length;k++)
              {
                ...
                var x =parseFloat(xyzArr2[0]);
                var y =parseFloat(xyzArr2[1]);
                var z =parseFloat(xyzArr2[2]);
                var pos = LocalToDegree(x,y,z);
                position.push(pos.x);
                position.push(pos.y);
              }
    ...
    openFlyAnimation() {
      ...
      mineBoundaryLocation.complete = function () {
        ...
        var animate3 = setTimeout(function () {
          ...
          _this.loadend = true
          if (viewer.layerList && viewer.layerList.length > 3 && viewer.layerList[3]._layers && viewer.layerList[3]._layers.length > 0 && viewer.layerList[3]._layers[0].config.type === 'animatelayer') { viewer.layerList[3]._layers[0].setVisible(true) }
      ...
      var _this = this
        ...
        // handler.setInputAction((click) => {
          ...
          if (height <9200 && height > 800) {
            ...
            //添加巷道高亮
            //if( window.vertexShader1 == ''){
                //g_hangdaoTileset.tileVisible.removeEventListener(_this.closePassageStyleTileset)
            //    //g_hangdaoTileset.tileVisible.addEventListener(_this.PassageStyleTileset)
           // }           
          }
          else if(height<800) {
            _this.openUndergroundMode()
          ...
          //else if(height <1000){
              //g_hangdaoTileset.tileVisible.removeEventListener(_this.PassageStyleTileset)
              //debugger;
              //g_hangdaoTileset.tileVisible.addEventListener(_this.closePassageStyleTileset)
    ...
    PassageStyleTileset(tile){
      ...
      void main(void) 
      {
        ...
        if (x >= 0.0)
        {
          tempx = acos(z / sqrt(x * x + z * z)) * 180.0 / 3.1415926535897932384626433832795;
          tempx = tempx < 0.0 ? (tempx + 180.0) : tempx;
        }
        else
        {
          tempx = acos(z / sqrt(x * x + z * z)) * 180.0 / 3.1415926535897932384626433832795;
        ...
        tempy = asin(y / sqrt(x * x + y * y + z * z)) * 180.0 / 3.1415926535897932384626433832795;
        tempy = tempy > 90.0 ? 90.0 - tempy : tempy;
        tempz = (sqrt(x * x + y * y + z * z)) - 6378137.0;
        v_positionEC2 = v_positionEC;
        v_positionEC2.x = a_position.x;
        v_positionEC2.y = a_position.y;
        v_positionEC2.z = a_position.z;
    ...
    var fragShader1 = `
        ...
        void main() 
        {
              gl_FragColor.rgba =vec4(0.0,0.5,1.0,1.0);
                    float stc_pl = fract(czm_frameNumber / 60.0) * 3.14159265 * 2.0;
                    float stc_sd = (v_positionEC2.z-200.0) / 500.0 + sin(stc_pl) * 0.1;
            float stc_sd_static = (v_positionEC2.z-200.0) / 500.0 ;
                    gl_FragColor *= vec4(stc_sd, stc_sd, stc_sd, 1.0);
                    float stc_a13 = fract(czm_frameNumber / 180.0);
                    float stc_h = clamp((v_positionEC2.z-200.0) / 1000.0, 0.0, 1.0);
    ...
    // 注入 shader
		    ...
		    var featureName;
				const feature = content.getFeature(i);
				{
					var proper = feature.content.batchTable._properties;
					 featureName = proper.newname[feature._batchId];
				}
				//var count = g_HangDaoNameMap[featureName];
				...
				model.colorBlendMode = Cesium.ColorBlendMode.HIGHLIGHT;
				 ...
				 if (  model && model._sourcePrograms && model._rendererResources) 
				 {
						 Object.keys(model._sourcePrograms).forEach(key => {
							...
							model._rendererResources.sourceShaders[program.vertexShader]=vertexShader1;
							...
							const color = 'vec4(1,0,0)'

							// 自定义着色器
						
							//model._rendererResources.sourceShaders[program.fragmentShader] = fragShader3;
						//	if(vertexShader1!=undefined)
						    model._rendererResources.sourceShaders[program.fragmentShader] = fragShader1;
						  });
						  	// 让系统重新编译着色器
    ...
    showCircleRipple(){
      ...
      var positions=[112.7145642,35.6762447,-275.34,
                      112.7025492,35.6797271,-346.60,
                      112.6626330,35.6871531,-490.95,
                      112.6696057,35.6800442,-451.29,
                      112.6960627,35.6943101,-390.61,
                      112.6764629,35.6840686,-428.83];
		  var j=0;
      ...
      { 
        ...
        for(let k=0;k<=num;k++)
        {
          ...
          var y =lat+ Math.sin(Math.PI/num*2*k)*0.00020;
          positions3.push(x);
          positions3.push(y);
        ...
        for(var i=0;i<=num;i++)
        {
          ...
          var y =lat+ Math.sin(Math.PI/num*2*i)*0.00010;
          positions2.push(x);
          positions2.push(y);
          positions2.push(alt);
        }
        ...
        for(var i=0;i<=num;i++)
        {
          var x =lon+ Math.cos(Math.PI/num*2*i)*0.00040;
        ...
        var positions5=[112.7303534,35.6715092,57,
        112.7305963,35.6720656,57,
        112.7291217,35.6724650,57,
        112.7288894,35.6718903,57,
        112.7303534,35.6715092,57];
        var options={"repeatX":1.0,"repeatY":1.0,"color":new Cesium.Color(1.0, 1.0, 1.0, 0.8)}
        ...
        var positions6=[112.7317426,35.6720780,45,
        112.7317956,35.6722028,45,
        112.7316237,35.6722481,45,
        112.7315750,35.6721219,45,
        112.7317426,35.6720780,45];
