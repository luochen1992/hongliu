  ...
  methods: {
    ...
    openFlyAnimation() {
      ...
      mineBoundaryLocation.complete = function () {
        ...
        var animate3 = setTimeout(function () {
          ...
          _this.loadend = true
          if (viewer.layerList && viewer.layerList.length > 3 && viewer.layerList[3]._layers && viewer.layerList[3]._layers.length > 0 && viewer.layerList[3]._layers[0].config.type === 'animatelayer')
    ...
    addFlowLinePoint() {
      ...
      for (var i = 0; i < pnts.length; i += 3) {
        ...
        let mm = _this.parabola([centerX, centerY, centerZ, pnts[i], pnts[i + 1], pnts[i + 2]])
    ...
    parabola(twoPoints) {
      // 抛物线绘制
      let s = []
      let startPoint = [twoPoints[0], twoPoints[1], twoPoints[2]] //起点的经度、纬度
      s = s.concat(startPoint)
      let step = 80 //线的多少，越多则越平滑(但过多浏览器缓存也会占用越多)
      let heightProportion = 0.25 //最高点和总距离的比值
      let dLon = (twoPoints[3] - startPoint[0]) / step //经度差值
      let dLat = (twoPoints[4] - startPoint[1]) / step //纬度差值
      ...
      let heigh = (step * Math.sqrt(deltaLon * deltaLon + deltaLat * deltaLat) * heightProportion).toFixed(0) * 2 + twoPoints[2]
